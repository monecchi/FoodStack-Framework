...
add_shortcode( 'malory_alert_block', 'ebor_alert_block_shortcode' );
...
function ebor_alert_block_shortcode_vc() {
	vc_map( 
		array(
			...
			"name" => esc_html__("Alert Bar", 'malory'),
			"base" => "malory_alert_block",
			"category" => esc_html__('malory WP Theme', 'malory'),
			'description' => 'An alert bar ideal for drawing attention.',
			"params" => array(
				array(
					"type" => "textarea_html",
					"heading" => esc_html__("Alert Content", 'malory'),
					"param_name" => "content",
					'holder' => 'div'
				),
				array(
